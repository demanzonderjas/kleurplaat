...
const DEFAULT_BACKGROUND = 'https://static.vecteezy.com/system/resources/thumbnails/003/439/678/original/cartoon-background-underwater-sea-life-free-video.jpg';

const SceneCanvas = ({ images = [], fillScreen, onProcessed, uploadedImage, background, onDuplicate, duplicatePositions = [], stickersList = [], onRemove, uids = [] }) => {
  ...
  // Bin sound
  const BIN_SOUND = '/sounds/bin.mp3';
  ...
  useEffect(() => {
    ...
    function handleClick(e) {
      ...
      for (let i = objStates.current.length - 1; i >= 0; i--) {
        ...
        if (!state) continue;
        if (
          px >= state.x && px <= state.x + objW &&
          py >= state.y && py <= state.y + objH
        ) {
          // Duplicate the image at this index, at the click/touch position, with stickers
          onDuplicate && onDuplicate(images[i], { x: px - objW / 2, y: py - objH / 2 }, JSON.parse(JSON.stringify(state.stickers || [])));
          break;
        }
      }
    }
  ...
  useEffect(() => {
    ...
    function animateObject(state, objW, objH) {
      ...
      const uploadBtnW = 340, uploadBtnH = 56;
      const uploadBtnRect = {
      ...
      for (let i = 0; i < objImgRefs.current.length; i++) {
        ...
        for (const sticker of stickers) {
          ...
          if (debugRef && debugRef.objectUid !== uid) {
            console.warn('Sticker', sticker.id, 'is not attached to the correct object!', { sticker, objectUid: uid, expectedUid: debugRef.objectUid });
          }
        }
        ctx.restore();
      }
  ...
  React.useEffect(() => {
    ...
    function handleStickerDrop() {
      ...
      for (let i = objStates.current.length - 1; i >= 0; i--) {
        ...
        if (isPointInRotatedRect(px, py, { x: state.x - STICK_MARGIN, y: state.y - STICK_MARGIN, angle: state.angle, w: marginW, h: marginH })) {
          topmostIndex = i;
          break;
        }
      }
